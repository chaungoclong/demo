...
<div class="main_content bg-white row m-0 pt-4">
	<div class="col-12">
		...
		<!-- nút thêm nhân viênvà thanh tìm kiếm -->
		<div class="row m-0 mb-3">
			...
			<div class="col-12 p-0 d-flex justify-content-between align-items-center bg-light p-2 border">
				...
				<div class="filter d-flex">
					...
					<!-- lọc quyền nhân viên-->
					<select id="filter_role" class='custom-select mr-3'>
						<option value="all" selected>Quyền: Tất cả</option>
						<option value="sa">Quyền: SuperAdmin</option>
						<option value="a">Quyền: Admin</option>
					</select>

					<!-- tìm kiếm tên , id khách, địa chỉ , ngày sinh... nhân viên-->
...
<script>
	$(function() {
		...
		// hoặc làm mới trang(lấy danh sách nhân viêntại trang đầu tiên vơi các tùy chọn tìm kiếm mặc định)
		fetchPageFirstTime();

		// lấy danh sách nhân viên khi nhập tìm kiếm
		$(document).on('input', '#search', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi lọc theo trạng thái
		$(document).on('change', '#filter_status', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi lọc theo quyền
		$(document).on('change', '#filter_role', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi lọc theo giới tính
		$(document).on('change', '#gender_opt', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi sắp xếp
		$(document).on('change', '#sort', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi thay đổi số hàng hiển thị
		$(document).on('change', '#number_of_rows', function() {
			fetchPage(1);
		});

		// lấy danh sách nhân viên khi chuyển trang
	...
	function fetchPage(currentPage = 1) {
		...
		let action = "fetch";
		let data = {q : q, status: status, role: role, sort: sort, gender: gender, numRows: numRows, currentPage: currentPage, action: action};
		let result = sendAJax("fetch_page.php", "post", "json", data);
		$('.list_user').html(result.users);
	...
	function changeStatus(btnID) {
		...
		let data = {userID: userID, active: active, action: action};
		let result = sendAJax("process_user.php", "post", "json", data);
		if(!result.ok) {
	...
	function deleteRow(btnID) {
		let userID = $(`#${btnID}`).data('user-id'); 
		let action = "delete";
		let data = {userID : userID, action: action};
		let result = sendAJax("process_user.php", "post", "json", data);
		console.log(result);
	...
	function setPrevPageData() {
		...
		localStorage.setItem("role", $('#filter_role').val());
